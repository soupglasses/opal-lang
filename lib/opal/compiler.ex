defmodule Opal.Compiler do
  ...
  def add_fun_clause(loc, {name, {:pattern, arg_list} = args, body}, env0) do
  ...
  # Cerl cannot handle clauses that cannot always match.
  ...
  defp generate_core([{{:=, loc}, lhs, rhs} | body], env0) do
    with {lhs_eval, env1} = generate_core(lhs, env0),
      ...
      # WARN: Assumes either full literal lhs, or single assignment.
  ...
  defp generate_core({:pattern, patterns}, env0) do
    {patterns_eval, env1} =
